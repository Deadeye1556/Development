...
3. Write the implementation directly: schema SQL, RLS policies, and service functions in your domain files
4. Run self-check and verify all acceptance criteria
5. Flag any Supabase dashboard actions the Board must execute (bucket creation, extension enables, or other dashboard-only operations that cannot be git-committed)
...
Schema / RLS / service functions written: [summary]
Tests run with actual output: [results, or BLOCKED + flagged dependency]
Board action required (dashboard SQL/RLS, if any): [list or "none"]
