...
**Cost target:** Under $0.02 per call
...
3. Test with at least 3 real inputs
4. Measure actual token cost per call
5. Verify fallback behavior triggers correctly when given bad input
6. Submit to Technology Manager with: pipeline files changed, sample inputs/outputs (3 examples), measured cost per call, fallback behavior verified
...
- If a real-world call exceeds cost target, flag to Technology Manager before merging
